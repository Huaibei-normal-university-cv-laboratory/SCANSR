...
from components.defineModel import defineG
...
class Trainer(object):
    def __init__(self, config, reporter):
        ...
        # ================urban100======================#
        self.eval_loader1 = dataloaderClass(config["dataset_name"],
                                            config["test_dataset_paths"][config["dataset_name"]],
                                            config["eval_batch_size"],
                                            image_scale=config["dataset_params"]["image_scale"],
                                            dataloader_num=config["dataset_params"]["dataloader_num"]
                                            )
    ...
    def __init_framework__(self):
        ...
        # print('model name: %s' % self.config["module_script_name"])
        ...
        self.network = defineG(self.config)

        # TODO replace below lines to define the model framework
        # self.network = network_class(1,
        #                              1,
        ...
        # print(self.config['CUDA_VISIBLE_DEVICES'])
        self.network = nn.DataParallel(self.network, device_ids=list(range(len(self.config['CUDA_VISIBLE_DEVICES'].split(',')))))
        # self.network = nn.DataParallel(self.network, device_ids=[0,1,2])
        ...
        if self.config["phase"] == "finetune":
            model_path = os.path.join('train_logs',self.config['version'],'checkpoints',
                                      "epoch%d_%s.pth" % (self.config["ckpt"],
                                                          self.config["checkpoint_names"]["generator_name"]))
            self.network.load_state_dict(torch.load(model_path), strict=False)
            print('loaded trained backbone model epoch {}...!'.format(model_path))

    # TODO modify this function to evaluate your model
    def __evaluation__(self, eval_loader, eval_iter, epoch, step=0,current_lr=0):
        ...
        if (dataset_name == "ixi") and (final_psnr > self.best_psnr["psnr"]):
            self.best_psnr["psnr"] = final_psnr
            self.best_psnr["epoch"] = epoch
        if (dataset_name == "BraTs2018") and (final_psnr > self.best_psnr["psnr"]):
            self.best_psnr["psnr"] = final_psnr
            self.best_psnr["epoch"] = epoch
            
        print("[{}], Epoch [{}], Dataset: {}, PSNR: {:.4f}, SSIM: {:.4f}".format(self.config["version"], epoch, dataset_name, final_psnr, final_ssim))
        self.reporter.writeTrainLog(epoch, step, "Dataset: {}, PSNR: {:.4f}, SSIM: {:.4f} Best PSNR: {:.4f} @ epoch {} lr: {:.4f}".format(dataset_name, final_psnr, final_ssim, self.best_psnr["psnr"], self.best_psnr["epoch"],current_lr))
        ...
        if dataset_name == "Set5":
            ...
            self.tensorboard_writer.add_scalar('metric/PSNR_ixi', final_psnr, epoch)
            print("[{}], Best ixi PSNR: {:.4f} @ epoch {}".format(self.config["version"],
                                                                  self.best_psnr["psnr"], self.best_psnr["epoch"]))
        elif dataset_name == "BraTs2018":
            self.tensorboard_writer.add_scalar('metric/PSNR_BraTs2018', final_psnr, epoch)
            print("[{}], Best BraTs2018 PSNR: {:.4f} @ epoch {}".format(self.config["version"],
                                                                  self.best_psnr["psnr"], self.best_psnr["epoch"]))
    ...
    def train(self):
        ...
        # import pdb; pdb.set_trace()
        # start = 1
        current_lr = self.config['optim_config']['lr']
        ...
        for epoch in range(start, total_epoch):
            ...
            for step in range(step_epoch):
                ...
                hr, lr_t2, hr_pd = self.train_loader.next()
                # input = [lr_t2, hr_pd]
                # print(lr_t2.shape)
                # print(hr_pd.shape)
                generated_hr = self.network(lr_t2, hr_pd)
                ...
                if (step + 1) % log_frep == 0:
                    ...
                    print("[{}], Elapsed [{}], Epoch [{}/{}], Step [{}/{}], loss: {:.4f}, l1: {:.4f}, lr: {:.4f}".
                          format(self.config["version"], elapsed, epoch + 1, total_epoch, step + 1, step_epoch,
                                 loss_curr.item(), loss_l1.item(),current_lr))
            ...
            if (epoch + 1) % model_freq == 0:
                ...
                self.__evaluation__(self.eval_loader1, self.eval_iter1, epoch + 1,current_lr=current_lr)
