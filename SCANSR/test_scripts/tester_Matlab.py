...
class Tester(object):
    ...
    def test(self):
        ...
        # save_dir = "./SR/"
        save_dir = os.path.join(base_root,"SR", "BI", self.config["version"],
        ...
        with torch.no_grad():
            for iii in tqdm(range(self.test_iter)):
                hr, lr_t2, hr_pd, names = self.test_loader()
                # print(lr.shape)
                if self.config["cuda"] >= 0:
                    hr = hr.cuda()
                    lr_t2 = lr_t2.cuda()
                ...
                if patch_test:
                    ...
                    # E = torch.zeros(b, c, h * scale, w * scale).type_as(lr_t2)
                    # W = torch.zeros_like(E)
                    #
                    # for h_idx in h_idx_list:
                    #     for w_idx in w_idx_list:
                    #         # in_patch = lr_t2[..., h_idx:h_idx + tile, w_idx:w_idx + tile],
                    #         h_idx_hr = h_idx * scale
                    #         w_idx_hr = w_idx * scale
                    #         in_hr = hr_pd[..., h_idx_hr:h_idx_hr + tile * scale, w_idx_hr:w_idx_hr + tile * scale]
                    #         out_patch = self.network(lr_t2[..., h_idx:h_idx + tile, w_idx:w_idx + tile], in_hr)
                ...
                for t in range(dataset_size):
                    ...
                    # temp_img = np.abs(temp_img)
                    # temp_img = cv2.normalize(temp_img, None, 0, 255, cv2.NORM_MINMAX)
                    i_name = names[t]
