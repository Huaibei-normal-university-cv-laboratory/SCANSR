...
def getParameters():
    ...
    # general settings
    parser.add_argument('-v', '--version', type=str, default='MyModule2_X4_ixi', help="version name for train, finetune")
    parser.add_argument('--train_yaml', type=str, default="train_MyModule2_X4_ixi.yaml")
    ...
    parser.add_argument('-e', '--ckpt', type=int, default=104,
...
def main():
    ...
    if config.phase == "train" or config.phase == "finetune":
        ...
        ymal_config = getConfigYaml(os.path.join(base_root, env_config["train_config_path"], config.train_yaml))
        for item in ymal_config.items():
            sys_state[item[0]] = item[1]
        # sys_state
        os.environ['CUDA_VISIBLE_DEVICES'] = sys_state['CUDA_VISIBLE_DEVICES']
        # create related dirs
        ...
        file1 = os.path.join(base_root,env_config["train_scripts_path"],
        ...
        # save the yaml file
        file1 = os.path.join(base_root,env_config["train_config_path"], config.train_yaml)
...
if __name__ == '__main__':
    main()
# 复现版本
# python train.py -v "OmniSR_X4_DIV2K_c1" -p train --train_yaml "train_OmniSR_X4_DIV2K.yaml" -c 1

# 服务器上的
# python /public/home/acay93af3w/Omni-SR-main/train.py -v "OmniSR_X4_DIV2K_c1" -p train --train_yaml "train_OmniSR_X4_DIV2K.yaml"

# 修改版本01 增加OCAB
# python train.py -v "OmniSR_X4_DIV2K_hat01" -p train --train_yaml "train_OmniSR_X4_DIV2K_hat01.yaml"

# 修改版本02 服务器上 增加OCAB 去掉注意力

# 消融
# python train.py -v "OmniSR_X4_DIV2K_hat01_window" -p train --train_yaml "train_OmniSR_X4_DIV2K_hat01.yaml"
