...
class DataPrefetcher():
    ...
    def __preload__(self):
        try:
            self.hr_t2, self.lr_t2, self.hr_t1 = next(self.dataiter)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.hr_t1 = self.hr_t1.cuda(non_blocking=True)
    ...
    def next(self):
        ...
        # ref = self.ref
        self.__preload__()
        return hr_t2, lr_t2, hr_t1

    def __len__(self):
        """Return the number of images."""
        return len(self.loader)

class BraTs_train(data.Dataset):
    def __init__(self, BraTs_root, scale, crop_size, **kwargs):
        super(BraTs_train, self).__init__()
        # path = opt['dataroot_GT']
        # hr_pd_path = os.path.join(ixi_root, 'train_HR_PD')
        hr_t2_path = os.path.join(BraTs_root, 'train_HR_T2')
        lr_t2_path = os.path.join(BraTs_root, "train_LR_T2", "X%d" % scale)
        hr_t1_path = os.path.join(BraTs_root, "train_HR_T1")
        ...
        if kwargs['dataloader_num'] == 'None':
            ...
            hr_t2s = sorted(os.listdir(hr_t2_path))
            lr_t2s = sorted(os.listdir(lr_t2_path))
            hr_t1s = sorted(os.listdir(hr_t1_path))
        else:
            ...
            hr_t1s = sorted(os.listdir(hr_t1_path))[:num]
        ...
        self.hr_t1s = [os.path.join(hr_t1_path, i) for i in hr_t1s]
        self.crop_size = crop_size

        self.scale = scale
        assert  len(hr_t2s) == len(lr_t2s) == len(hr_t1s), 'ref image number != hr image number'
        print('total train image number(lr_t2):%s' % len(lr_t2s))
        print('total train image number(hr_t1):%s' % len(hr_t1s))
    ...
    def __getitem__(self, idx):
        ...
        hr_t1 = cv2.imread(self.hr_t1s[idx], cv2.IMREAD_UNCHANGED)
        ...
        hr_t1 = torch.tensor(hr_t1).unsqueeze(0).float() / 255.
        ...
        hr_t1 = hr_t1[:, rnd_h_HR:rnd_h_HR + self.crop_size * self.scale, rnd_w_HR:rnd_w_HR + self.crop_size * self.scale]
...
def GetLoader(dataset_roots,
    ...
    content_dataset = BraTs_train(dataset_roots, image_scale, lr_patch_size, **kwargs)
...
if __name__ == '__main__':
    print("=")
    train_dataset = '/data2/cwj/datasets/BraTs2018/'
