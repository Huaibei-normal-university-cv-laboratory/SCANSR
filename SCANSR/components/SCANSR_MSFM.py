...
class DCAT(nn.Module):
    ...
    def __init__(
        ...
        self.epa_block = DCA(input_size=input_size, input_size1=input_size, hidden_size=hidden_size,
                             proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,
...
class DCA(nn.Module):
    ...
    def forward(self, x, ref):
        ...
        index = torch.topk(attn_CA, k=int(C / 2), dim=-1, largest=True)[1]
        mask1.scatter_(-1, index, 1.)
        attn1 = torch.where(mask1 > 0, attn_CA, torch.full_like(attn_CA, float('-inf')))

        index = torch.topk(attn_CA, k=int(C * 2 / 3), dim=-1, largest=True)[1]
        mask2.scatter_(-1, index, 1.)
        attn2 = torch.where(mask2 > 0, attn_CA, torch.full_like(attn_CA, float('-inf')))

        index = torch.topk(attn_CA, k=int(C * 3 / 4), dim=-1, largest=True)[1]
        mask3.scatter_(-1, index, 1.)
        attn3 = torch.where(mask3 > 0, attn_CA, torch.full_like(attn_CA, float('-inf')))

        index = torch.topk(attn_CA, k=int(C * 4 / 5), dim=-1, largest=True)[1]
        mask4.scatter_(-1, index, 1.)
        attn4 = torch.where(mask4 > 0, attn_CA, torch.full_like(attn_CA, float('-inf')))
        ...
        x_CA = out.permute(0, 3, 1, 2).reshape(B, N, C)

        ################################################3
        # attn_CA = (q_shared @ k_shared.transpose(-2,
        #                                          -1)) * self.temperature  # (B,Head,C,N) * (#B,Head,N,C) -> (B,Head,C,C)
        #
        ...
        x_SA = (attn_SA @ v_SA_projected.transpose(-2, -1)).permute(0, 3, 1, 2).reshape(B, N,
                                                                                        C)  # (B,Head,N,64) * (B,Head,64,C) -> (B,Head,N,C) -> (B,N,C)
...
class Decoder(nn.Module):
    ...
    def forward(self, lr_l, warp_ref_l):
        ...
        fea_x1 = self.act(self.merge_warp_x1(torch.cat([warp_ref_l[2], fea_L1], dim=1)))
        fea_x1 = self.blk_x1(fea_x1)
        fea_x1_up = self.up2(fea_x1) #(1, 128, 128, 128)

        # warp_ref_x2 = self.pAda2(fea_x1_up, warp_ref_l[1])
        fea_x2 = self.act(self.merge_warp_x2(torch.cat([warp_ref_l[1], fea_x1_up], dim=1)))
        ...
        fea_x4 = self.act(self.merge_warp_x4(torch.cat([warp_ref_l[0], fea_x2_up], dim=1)))
...
if __name__=='__main__':
    net = SCANSR_MSFM(upscale=2)
    # net.test()
    input1_tensor = torch.rand(1, 1, 64, 64)
    input2_tensor = torch.rand(1, 1, 128, 128)
