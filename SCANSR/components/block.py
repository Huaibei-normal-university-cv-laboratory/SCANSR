...
class MixedAttentionLayer(nn.Module):
    def __init__(self, channel, reduction=16, kernel_size=7):
        ...
        self.conv_spatial = nn.Sequential(
            nn.Conv2d(channel, 1, kernel_size, padding=kernel_size // 2, bias=False),
            nn.Sigmoid()
        )
    def forward(self, x):
        y_channel = self.conv_channel(self.contrast(x) + self.avg_pool(x))  # 计算通道注意力
        y_spatial = self.conv_spatial(x)  # 计算空间注意力
        # 组合通道注意力和空间注意力（这里使用乘法）
        y = x * y_channel * y_spatial
        return y
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        y = self.max_pool(x) + self.avg_pool(x)
        y = self.conv_du(y)
        return x * y
...
class IMDModule(nn.Module):
    def __init__(self, in_channels, distillation_rate=0.25):
        ...
        # self.cca = MixedAttentionLayer(self.distilled_channels * 4)  # Next4
        # self.cca = CCALayer(self.distilled_channels * 4)  # My4
        self.cca = ChannelAttention(self.distilled_channels * 4)  # Our4
...
# 修改了渐进细化块和注意力层
class IMDModule_New(nn.Module):
    def __init__(self, in_channels, distillation_rate=0.125):
        super(IMDModule_New, self).__init__()
        self.distilled_channels = int(in_channels * distillation_rate)
        self.remaining_channels = int(in_channels - self.distilled_channels)
        self.c1 = conv_layer(in_channels, in_channels, 3)
        self.c2 = conv_layer(self.remaining_channels, in_channels, 3)
        ...
        self.cam = ChannelAttention(self.distilled_channels * 8)
    ...
    def forward(self, input):
        ...
        out_c3 = self.act(self.c3(remaining_c2))
        distilled_c3, remaining_c3 = torch.split(out_c3, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c4 = self.act(self.c3(remaining_c3))
        distilled_c4, remaining_c4 = torch.split(out_c4, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c5 = self.act(self.c3(remaining_c4))
        distilled_c5, remaining_c5 = torch.split(out_c5, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c6 = self.act(self.c3(remaining_c5))
        distilled_c6, remaining_c6 = torch.split(out_c6, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c7 = self.act(self.c3(remaining_c6))
        distilled_c7, remaining_c7 = torch.split(out_c7, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c8 = self.c4(remaining_c7)
        out = torch.cat([distilled_c1, distilled_c2, distilled_c3, distilled_c4, distilled_c5, distilled_c6, distilled_c7, out_c8], dim=1)
        # out_fused = self.c5(self.cca(out)) + input
        out_fused = self.c5(self.cam(out)) + input
        return out_fused
...
if __name__=='__main__':
    net = IMDModule_New(in_channels=64)
    # net.test()
    input1_tensor = torch.rand(1, 64, 256, 256)
    out = net(input1_tensor)
    print(out.shape)
